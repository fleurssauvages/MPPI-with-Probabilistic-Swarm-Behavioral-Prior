...
def densify_states_for_collision(states: Array, max_xy_step: float = 0.035) -> Array:
    """Densify a state trajectory for swept collision checking.

    This catches thin wall crossings between two discrete rollout nodes. Heading
    is carried only for array shape; clearance uses x,y.
    """
    ...
    for a, b in zip(X[:-1], X[1:]):
        ...
        n = max(1, int(math.ceil(dist / max(float(max_xy_step), 1e-9))))
...
def trajectory_has_hard_collision(
    ...
    *,
    ...
    Uses densified/swept exact polygon clearance, not only rollout nodes.
    """
    if not enabled or X is None or len(obstacles) == 0:
        return False
    dense = densify_states_for_collision(X)
    return bool(min_clearance(dense, obstacles, robot_radius) < float(clearance_buffer))
...
def add_hard_collision_penalties(
    ...
    *,
    ...
    """Reject predicted rollouts that collide with exact active obstacles.

    This is a hard feasibility layer on top of the smooth obstacle cost. Any
    densified rollout whose exact polygon clearance is below clearance_buffer
    receives a very large additive cost. X must have shape (N,H+1,3).
    """
    if not enabled or X is None or len(obstacles) == 0:
        return costs
...
def hard_collision_free_trajectory(
    X: Array,
    obstacles: Sequence,
    robot_radius: float,
    *,
    ...
    return not trajectory_has_hard_collision(
        X,
        obstacles,
        robot_radius,
        enabled=True,
...
def emergency_safe_control(
    ...
    *,
    ...
    """Return a one-step collision-free fallback command if possible."""
    candidates = [
        (0.0, 0.0),
    ...
    best = None
    for v, om in candidates:
        u = np.array([
            np.clip(v, cfg.v_min, cfg.v_max),
            np.clip(om, cfg.omega_min, cfg.omega_max),
        ], dtype=np.float64)
        X = rollout_unicycle(x_current, u.reshape(1, 2), dt)
        if not hard_collision_free_trajectory(
            X,
            obstacles,
            cfg.robot_radius,
            clearance_buffer=getattr(cfg, "hard_collision_clearance_buffer", 0.0),
        ):
            continue

        dist_goal = float(np.linalg.norm(X[-1, :2] - goal))
        effort = 0.05 * float(u[0] * u[0] + 0.15 * u[1] * u[1])
        score = dist_goal + effort
        if best is None or score < best[0]:
            best = (score, u, X)

    if best is not None:
        return best[1].copy(), {
            "safety_shield": "emergency_safe_control",
            "optimal_traj": best[2],
        }

    return np.array([0.0, 0.0], dtype=np.float64), {
        "safety_shield": "no_safe_one_step_control_found",
        "optimal_traj": rollout_unicycle(x_current, np.zeros((1, 2), dtype=np.float64), dt),
    }
...
def shield_control_if_unsafe(
    ...
    *,
    ...
    """Replace an applied command if its swept one-step motion collides."""
    if not getattr(cfg, "enable_hard_collision_filter", True):
        return u, None

    X = rollout_unicycle(x_current, np.asarray(u, dtype=np.float64).reshape(1, 2), dt)
    if hard_collision_free_trajectory(
        X,
        obstacles,
        cfg.robot_radius,
        clearance_buffer=getattr(cfg, "hard_collision_clearance_buffer", 0.0),
    ...
    safe_u, shield_info = emergency_safe_control(x_current, obstacles, goal, cfg, dt=dt)
    shield_info["unsafe_requested_control"] = np.asarray(u, dtype=np.float64).copy()
    return safe_u, shield_info


def safe_mppi_control_from_rollouts(
    costs: Array,
    U: Array,
    X: Array,
    x_current: Array,
    ...
    """Choose a hard-feasible MPPI action.

    Weighted averaging can leave the feasible rollout set in nonlinear dynamics.
    In hard mode, use the first control of the lowest-cost feasible rollout.
    """
    if not cfg.enable_hard_collision_filter:
        return mppi_weighted_control(costs, U[:, 0, :], cfg), {
            "mppi_action_rule": "weighted_average_no_hard_filter",
            "optimal_traj": best_output_trajectory_from_costs(costs, X),
        }

    feasible = np.zeros(X.shape[0], dtype=bool)
    for i in range(X.shape[0]):
        feasible[i] = hard_collision_free_trajectory(
            X[i],
            obstacles,
            cfg.robot_radius,
    ...
    if np.any(feasible):
        feasible_costs = np.asarray(costs, dtype=np.float64).copy()
        feasible_costs[~feasible] = np.inf
        best_idx = int(np.argmin(feasible_costs))
        return U[best_idx, 0, :].copy(), {
            "mppi_action_rule": "best_feasible_rollout_first_control",
            "num_feasible_rollouts": int(np.sum(feasible)),
            "optimal_traj": np.asarray(X[best_idx], dtype=np.float64).copy(),
        }

    safe_u, shield_info = emergency_safe_control(x_current, obstacles, goal, cfg, dt=cfg.dt)
    shield_info["mppi_action_rule"] = "emergency_no_feasible_rollouts"
    shield_info["num_feasible_rollouts"] = 0
    return safe_u, shield_info
...
def stable_swarm_mppi_step(
    ...
    *,
    ...
    u, action_info = safe_mppi_control_from_rollouts(
    ...
    info = {
        ...
        "optimal_traj": action_info.get("optimal_traj", best_traj),
        **action_info,
    }
...
def mode_selecting_stable_mppi_step(
    ...
    *,
    ...
    for mid, global_mode in enumerate(candidate_modes):
        ...
        u_h, action_info = safe_mppi_control_from_rollouts(
            costs, U, X, x_current, obstacles, goal, cfg
        )
        feasible_count = int(action_info.get("num_feasible_rollouts", X.shape[0]))
        J_h = softmin_score(costs, cfg) if feasible_count > 0 else float("inf")
        ...
        if best is None or J_h < best["score"]:
            ...
            best = {
                ...
                "optimal_traj": action_info.get("optimal_traj", np.asarray(X[best_idx], dtype=np.float64).copy()),
                "mppi_action_rule": action_info.get("mppi_action_rule"),
                "num_feasible_rollouts": feasible_count,
            }
    ...
    info = {
        ...
        "num_feasible_rollouts": best.get("num_feasible_rollouts"),
    }
...
def run_controller_variant(
    ...
    *,
    ...
    for _ in range(max_steps):
        ...
        shield_u, shield_info = shield_control_if_unsafe(
            x, u, obstacles, goal, mppi_cfg, dt=mppi_cfg.dt
        )
        ...
        shield_u, shield_info = shield_control_if_unsafe(
            x, u, active_obstacles, goal, mppi_cfg, dt=mppi_cfg.dt
        )
        if shield_info is not None:
            u = shield_u
            info.update(shield_info)
...
def main_dynamic_blockage():
    ...
    if pd is not None:
        ...
        print("Saved metrics: dynamic_block_hard_metrics.csv")
        print(df)
    else:
        for row in rows:
            print(row)
