#!/usr/bin/env python3
"""
Compare swarm-prior MPPI variants against standard MPC/MPPI baselines.

This script is designed for the same project setup as your original
probabilistic(3).py script. It runs:

    1. FULL_SWARM_PRIOR_MPPI
       MPPI with:
           pi_h homotopy prior
           empirical swarm rollout initializations
           mu_h reference
           Sigma_h Mahalanobis tracking
           Sigma_h uncertainty-aware obstacle margin

    2. GAUSSIAN_PRIOR_MPPI
       MPPI with:
           pi_h, mu_h, Sigma_h
       but no empirical swarm rollout initialization.

    3. EMPIRICAL_INIT_MPPI
       MPPI with:
           empirical swarm rollout initialization
       but no Sigma_h Mahalanobis tracking and no pi_h prior penalty.

    4. STANDARD_MPPI
       No swarm prior, no homotopy modes.
       Samples noisy controls around a goal-directed unicycle nominal policy.

    5. STANDARD_MPC
       No MPPI.
       Uses scipy.optimize.minimize over a single unicycle control sequence.

    6. HOMOTOPY_MEAN_MPC
       No MPPI.
       Runs a standard deterministic MPC for each homotopy mean path and chooses
       the best feasible/lowest-cost control. This uses the swarm mean reference,
       but not stochastic MPPI sampling.
...
Run:
    python compare_swarm_mppi_mpc_variants.py

Notes:
    - The swarm part can be expensive. Set RUN_SWARM_ONCE=True and keep results
      in memory for all variants.
    - The MPPI loops are vectorized with bounding-circle obstacle costs.
    - The MPC baselines use scipy.optimize.minimize. If scipy is unavailable,
      the script raises a clear error when running MPC variants.
"""
...
class MPPIConfig:
    ...
    # When enabled, any predicted horizon that intersects an active obstacle is
    # rejected by adding a very large penalty. The normal obstacle term remains
    # as a smooth shaping cost, but feasibility is decided by this exact check.
    enable_hard_collision_filter: bool = True
    hard_collision_penalty: float = 1.0e9
...
class MPCConfig:
    ...
    # Hard collision safety layer for deterministic MPC.
    enable_hard_collision_filter: bool = True
    hard_collision_penalty: float = 1.0e9
    hard_collision_clearance_buffer: float = 0.0
...
# Parameters with the same physical/cost meaning in MPPIConfig and MPCConfig.
# Horizon, MPPI sampling parameters, covariance/corridor terms, and optimizer
# settings are intentionally excluded because they are controller-specific.
SHARED_MPPI_MPC_PARAMETER_NAMES = (
    "dt",
    "v_min",
    "v_max",
    "omega_min",
    ...
    "w_control",
    "w_control_smooth",
    "enable_hard_collision_filter",
    "hard_collision_penalty",
    "hard_collision_clearance_buffer",
)


def check_shared_mppi_mpc_parameters(
    mppi_cfg: MPPIConfig,
    mpc_cfg: MPCConfig,
    *,
    atol: float = 1e-12,
) -> bool:
    """Print and validate parameters shared by the MPPI and MPC costs/dynamics."""
    mismatches = []
    print("Shared MPPI/MPC parameter check:")
    for name in SHARED_MPPI_MPC_PARAMETER_NAMES:
        mppi_value = getattr(mppi_cfg, name)
        mpc_value = getattr(mpc_cfg, name)
        same = bool(np.isclose(float(mppi_value), float(mpc_value), atol=atol, rtol=0.0))
        status = "OK" if same else "MISMATCH"
        print(f"  {name}: MPPI={mppi_value} MPC={mpc_value} [{status}]")
        if not same:
            mismatches.append(name)

    if mismatches:
        print("WARNING: shared MPPI/MPC parameters differ: " + ", ".join(mismatches))
        return False
    return True
...
def add_hard_collision_penalties(
    costs: Array,
    X: Array,
    obstacles: Sequence,
    robot_radius: float,
    *,
    ...
    """Reject predicted rollouts that collide with the exact active obstacles.

    The regular obstacle term is a smooth penalty and can be outweighed by goal
    or reference tracking. This function adds a hard feasibility layer: any
    rollout whose exact polygon clearance is below clearance_buffer receives a
    very large additive cost. X is expected to have shape (N,H+1,3).
    """
    if not enabled or X is None or len(obstacles) == 0:
        return costs

    costs = np.asarray(costs, dtype=np.float64).copy()
    for i in range(X.shape[0]):
        if min_clearance(X[i], obstacles, robot_radius) < float(clearance_buffer):
            costs[i] += float(penalty)
    return costs


def trajectory_has_hard_collision(
    X: Array,
    obstacles: Sequence,
    robot_radius: float,
    *,
    ...
    """Return True when a predicted deterministic trajectory is infeasible."""
    if not enabled or X is None or len(obstacles) == 0:
        return False
    return bool(min_clearance(X, obstacles, robot_radius) < float(clearance_buffer))
...
def stable_swarm_mppi_step(
    ...
    *,
    ...
    for mid, local_mode in enumerate(local_modes):
        ...
        costs = add_hard_collision_penalties(
            costs,
            X,
            obstacles,
            cfg.robot_radius,
            enabled=cfg.enable_hard_collision_filter,
            penalty=cfg.hard_collision_penalty,
...
def solve_standard_mpc_once(
    ...
    def objective(z):
        ...
        if trajectory_has_hard_collision(
            X,
            obstacles,
            cfg.robot_radius,
            enabled=cfg.enable_hard_collision_filter,
            ...
            # Add a small smooth residual to keep deterministic optimizers from
            # treating all infeasible controls as exactly identical.
            return float(cfg.hard_collision_penalty) + mpc_stage_cost_fast_circles(
                X, U, obstacle_circles, goal, cfg, ref_path=ref_path
            )
    ...
    X_star = rollout_unicycle(x_current, U_star, cfg.dt)
    u = U_star[0].copy()
    u[0] = np.clip(u[0], cfg.v_min, cfg.v_max)
    u[1] = np.clip(u[1], cfg.omega_min, cfg.omega_max)
...
def run_dynamic_blockage_controller(
    ...
    *,
    ...
    # Warm starts for deterministic MPC variants under dynamic obstacles.
    standard_mpc_warm = None
    homotopy_mpc_warm = {}
    ...
    for step in range(max_steps):
        ...
        if variant == ControllerVariant.FULL_SWARM_PRIOR_MPPI:
            ...
            obstacle_circles = obstacle_bounding_circles(active_obstacles)
            u, info, standard_mpc_warm = standard_mpc_step(
                x,
                active_obstacles,
                goal,
        ...
        dt = mppi_cfg.dt if "mppi" in variant.value else mpc_cfg.dt
...
def _mode_index_from_info(info: Dict[str, object], modes: List[MPPIHomotopyMode]) -> Optional[int]:
    ...
    For HOMOTOPY_MEAN_MPC, info stores the signature string rather than an index.
    """
...
def tracked_reference_for_frame(
    ...
    *,
    ...
    the visual reference from the recorded state and controller info:
      ...
      - HOMOTOPY_MEAN_MPC uses its recorded mode signature;
    ...
    if variant_name in {
        ...
        ControllerVariant.STANDARD_MPC.value,
    }:
    ...
    H = mpc_cfg.horizon if "mpc" in variant_name and "mppi" not in variant_name else mppi_cfg.horizon
...
def animate_dynamic_blockage_one_variant(
    ...
    *,
    ...
    def update(frame):
        ...
        dt_for_arrows = (
            mppi_cfg.dt
            if "mppi" in str(result.get("variant", ""))
            else mpc_cfg.dt
        )
...
def animate_dynamic_blockage_by_variant(
    ...
    goal,
    bounds,
    output_dir="dynamic_wall_blockage_variant_gifs",
    *,
    mppi_cfg: Optional[MPPIConfig] = None,
    mpc_cfg: Optional[MPCConfig] = None,
):
    ...
    for res in results:
        filename = f"dynamic_wall_blockage_{safe_variant_filename(res['variant'])}_tracked_reference.gif"
...
def main_dynamic_blockage():
    print("Dynamic blockage homotopy-switch experiment")
    ...
    mpc_cfg = MPCConfig(
        ...
        base_safety_margin=cfg.base_safety_margin,
        w_goal=cfg.w_goal,
        w_obstacle=cfg.w_obstacle,
        w_control=cfg.w_control,
        w_control_smooth=cfg.w_control_smooth,
        enable_hard_collision_filter=cfg.enable_hard_collision_filter,
        hard_collision_penalty=cfg.hard_collision_penalty,
        hard_collision_clearance_buffer=cfg.hard_collision_clearance_buffer,
        maxiter=12,
        use_fast_obstacle_circles=True,
        top_k_homotopy_mpc=2,
        warm_start=True,
    )
    check_shared_mppi_mpc_parameters(cfg, mpc_cfg)
    ...
    print(f"Hard collision filter: {cfg.enable_hard_collision_filter}, penalty={cfg.hard_collision_penalty:.1e}, clearance_buffer={cfg.hard_collision_clearance_buffer}")
    ...
    if pd is not None:
        ...
        df.to_csv("dynamic_wall_blockage_metrics.csv", index=False)
        print("Saved metrics: dynamic_wall_blockage_metrics.csv")
        print(df)
    else:
        for row in rows:
            print(row)

    # Intentionally do not save the aggregate path image or aggregate GIF.
    # Only per-variant GIFs are written below.
...
def main():
    print("Variant file: dynamic_wall_blockage")
    print(f"Numba enabled: {njit is not None}")
    ...
    mpc_cfg = MPCConfig(
        horizon=12,
        dt=mppi_cfg.dt,
        v_min=mppi_cfg.v_min,
        v_max=mppi_cfg.v_max,
        omega_min=mppi_cfg.omega_min,
        omega_max=mppi_cfg.omega_max,
        robot_radius=mppi_cfg.robot_radius,
        base_safety_margin=mppi_cfg.base_safety_margin,
        w_goal=mppi_cfg.w_goal,
        w_obstacle=mppi_cfg.w_obstacle,
        w_control=mppi_cfg.w_control,
        w_control_smooth=mppi_cfg.w_control_smooth,
        maxiter=12,
        use_fast_obstacle_circles=True,
        top_k_homotopy_mpc=2,
        warm_start=True,
    )
    check_shared_mppi_mpc_parameters(mppi_cfg, mpc_cfg)
