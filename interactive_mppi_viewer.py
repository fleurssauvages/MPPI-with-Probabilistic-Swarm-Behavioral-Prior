...
The viewer loads one controller module without executing its batch entry point.
It lets the user select the controller variant, condition, wall scenario,
controller seed, swarm seed, rollout count, and playback speed.

A selected trial is simulated once in a worker thread and then animated in a
Tk window. The animation shows the exact obstacles, optional collision
representation, robot pose, executed trajectory, current MPPI output trajectory,
and the prior used by the selected controller.
"""
...
class InteractiveMPPIViewer:
    ...
    def _get_module(self) -> Any:
        if self.module is None:
            self.module = load_python_module(self.module_path, "interactive_mppi_engine")
    ...
    def _draw_robot(self, state: np.ndarray) -> None:
        ...
        self.ax.add_patch(
            Circle((x, y), radius=radius, facecolor="#17becf", edgecolor="black", linewidth=1.0, zorder=12)
        )
