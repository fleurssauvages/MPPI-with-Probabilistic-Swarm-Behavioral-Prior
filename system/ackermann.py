...
if njit is not None:
    ...
    @njit(cache=True)
    def centerline_connection_clearance_nb(start_state, end_point, circle_centers, circle_radii, vehicle_length, vehicle_width):
        if circle_radii.shape[0] == 0:
            return 1e18
        sx = start_state[0]
        sy = start_state[1]
        ...
        if length <= 1e-12:
            return minimum_rectangle_circle_clearance_nb(
                start_state,
                circle_centers,
                circle_radii,
                vehicle_length,
        ...
        half_length = 0.5 * vehicle_length
        half_width = 0.5 * vehicle_width
        ax = sx - half_length * ux
        ay = sy - half_length * uy
        bx = ex + half_length * ux
        by = ey + half_length * uy
        abx = bx - ax
        aby = by - ay
        ...
        for index in range(circle_radii.shape[0]):
            ...
            qx = ax + alpha * abx
            qy = ay + alpha * aby
            cx = circle_centers[index, 0] - qx
            cy = circle_centers[index, 1] - qy
            clearance = math.sqrt(cx * cx + cy * cy) - circle_radii[index] - half_width
            if clearance < best:
                best = clearance
        return best
else:
    ...
    _dynamic_ackermann_step_nb = None
    ...
    rectangle_circle_clearance_nb = None
    ...
    point_in_oriented_rectangle_nb = None
    point_segment_dist_nb = None
    orientation_2d_nb = None
    point_on_segment_nb = None
    segments_intersect_nb = None
    segment_segment_distance_nb = None
    centerline_connection_clearance_nb = None
    rectangle_polygon_clearance_nb = None
...
def centerline_connection_clearance(
    ...
    """Clearance of the straight connection from the vehicle to a mode centerline."""
    if not obstacle_circles:
        return 1e309
    centers, radii = obstacle_circles_to_arrays(obstacle_circles)
    state = np.asarray(x_current, dtype=np.float64)
    ...
    distance = float(np.linalg.norm(delta))
    if distance <= 1e-12:
        return minimum_rectangle_circle_clearance(
            state,
            centers,
            radii,
            cfg.vehicle_length,
    ...
    a = state[:2] - 0.5 * float(cfg.vehicle_length) * direction
    b = point + 0.5 * float(cfg.vehicle_length) * direction
    ab = b - a
    denominator = float(ab @ ab)
    alpha = np.clip(((centers - a[None, :]) @ ab) / denominator, 0.0, 1.0)
    closest = a[None, :] + alpha[:, None] * ab[None, :]
    return float(np.min(np.linalg.norm(centers - closest, axis=1) - radii - 0.5 * float(cfg.vehicle_width)))
